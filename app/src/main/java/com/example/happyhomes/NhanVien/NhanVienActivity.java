...
public class NhanVienActivity extends AppCompatActivity {
    ...
    private void showReadyToWorkDialog() {
        ...
        btnCheckIn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                saveCheckInData(); // 1 là mã cho check-in
            }
        });
    }
    ...
    private void saveCheckInData() {
        ...
        // Lấy danh sách các Workdate dựa trên EMID
        List<Workdate> workdates = dbHelper.getWorkdatesByEmployeeId(employee.getEmId());

        Workdate matchingWorkdate = null;
        Schedule matchingSchedule = null;

        // Lấy thời gian hiện tại chỉ gồm giờ, phút, giây
        String currentTimeString = new SimpleDateFormat("HH:mm:ss", Locale.getDefault()).format(new Date());

        // Duyệt qua các Workdate để tìm Ser_Sche_ID phù hợp
        for (Workdate workdate : workdates) {
            Schedule schedule = dbHelper.getScheduleBySerScheId(workdate.getSerScheId());

            if (schedule != null) {
                // Lấy StartTime từ Schedule chỉ gồm giờ, phút, giây
                String scheduleTimeString = new SimpleDateFormat("HH:mm:ss", Locale.getDefault()).format(schedule.getStartTime());

                // So sánh giờ, phút, giây
                if (isTimeWithinOneHour(currentTimeString, scheduleTimeString)) {
                    matchingWorkdate = workdate;
        ...
        if (matchingWorkdate != null && matchingSchedule != null) {
            // Chuyển đổi thời gian hiện tại thành String
            String currentTimeAsString = new SimpleDateFormat("HH:mm:ss", Locale.getDefault()).format(new Date());

            // Tạo đối tượng Check_Work với workdateId lấy từ Workdate phù hợp
            Check_Work checkWork = new Check_Work(null, matchingWorkdate.getWorkDateId(), null, 1, currentTimeAsString); // 1 đại diện cho Check-In
            ...
            // Lưu Check_Work vào cơ sở dữ liệu
            dbHelper.addCheckWork(checkWork);
        } else {
            // Xử lý khi không tìm thấy Workdate hoặc Schedule phù hợp
            AlertDialog.Builder builder = new AlertDialog.Builder(NhanVienActivity.this);
            builder.setTitle("Thông báo");
            builder.setMessage("Hiện tại ngoài giờ làm việc của bạn.");
            ...
            alertDialog.show();
            Log.e("NhanVienActivity", "Không tìm thấy Workdate hoặc Schedule phù hợp cho nhân viên với ID: " + employee.getEmId());
        }
    }

    private boolean isTimeWithinOneHour(String currentTimeString, String scheduleTimeString) {
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss", Locale.getDefault());
            Date currentTime = sdf.parse(currentTimeString);
            Date scheduleTime = sdf.parse(scheduleTimeString);

            // Kiểm tra nếu thời gian hiện tại nằm trong khoảng 1 giờ sau StartTime
            return currentTime != null && scheduleTime != null &&
                    currentTime.after(scheduleTime) && currentTime.before(new Date(scheduleTime.getTime() + 3600000));
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
